...
func ServeVoiceChatStream(userID uint64, fullname string, w http.ResponseWriter, r *http.Request) {
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		log.Println(err)
		return
	...
	if streamvoicechathub == nil {
		streamvoicechathub = NewHub()
		streamvoicechathub.Run()
	}

	client := &Client{UserID: userID, Fullname: fullname, hub: streamvoicechathub, conn: conn, send: make(chan []byte, 4096)}
	client.hub.register <- client
